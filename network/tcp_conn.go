...
func newTCPConn(conn net.Conn, pendingWriteNum int, msgParser *MsgParser, writeDeadline time.Duration, logger log.ILogger) *TCPConn {
	if logger == nil {
		panic("tcp connect required a logger")
	}
	tcpConn := new(TCPConn)
	tcpConn.conn = conn
...
func (tcpConn *TCPConn) doWrite(b []byte) error {
	if len(tcpConn.writeChan) == cap(tcpConn.writeChan) {
		...
		tcpConn.logger.Error("close conn: channel full")
		tcpConn.doDestroy()
